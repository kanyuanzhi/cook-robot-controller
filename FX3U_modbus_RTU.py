...
class modbus_RTU_communication():
    ...
    def read_register(self, datas):
        '''
        n个数据寄存器D读：站号；功能码03；寄存器地址；寄存器个数；CRC
        读D123、D124：01 03 00 7B 00 02 B4 12
        Returns:
        写入成功，返回True
        '''
        if len(datas) == 0:
...
if __name__ == '__main__':
    ...
    if ser != False:
        ...
        dataRead = [['D1', 1], ['D123', 2]]
        resultRead = sl_rtu.read_register(dataRead)
        print(resultRead)

        # 向数据寄存器D100开始3个寄存器，写入值（16位整数格式）
        dataWrite = [['D100', 1], ['D101', 2], ['D102', 3]]
        sl_rtu.write_register(dataWrite)
