...
def fire_control(action: bytes, fire_level: int = 0, execute_time: float = 0, is_immediate=False):
    ...
    # fire_level：0~10档，加热温度0~200℃，1档20℃
    if fire_level == 0:
        temperature = 0
    else:
        temperature = fire_level * 200
...
def reset1_control(execute_time: float = 0, is_immediate=False):
...
def wash_control(execute_time: float = 0):
    ...
    temperature_control(b"\x01", 500, execute_time=execute_time + 15)  # 加热50℃
    temperature_control(b"\x01", 0, execute_time=execute_time + 60)  # 10秒后停止加热
    r_control(b"\x02", execute_time=execute_time + 65)  # R轴停转
    y_control(b"\x01", 7, execute_time=execute_time + 65)  # Y轴定位7号位（倒水位）
    y_control(b"\x01", 1, execute_time=execute_time + 70)  # Y轴定位1号位（接料1位）
