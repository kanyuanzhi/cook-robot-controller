...
def seasoning_control(slot_num: int, weight: int, execute_time: float = 0):
    ...
    if slot_num in [1, 2, 3, 4, 5, 6]:  # 液体泵
        ...
        time = weight * 10 / 4  # 加料分量与加料时长的转换关系,1ml/g~0.1s
...
def dish_out_control(execute_time: float = 0):
    ...
    shake_control(5, execute_time=execute_time + 3)  # todo： 一键抖菜，抖5次，需要测试
...
def reset0_control(execute_time: float = 0):
    print("****复位0****执行时刻{}s".format(execute_time))
    x_control(b"\x01", 0, execute_time=execute_time)  # X轴定位0号位（原点）
    y_control(b"\x01", 2, execute_time=execute_time)  # Y轴定位9号位（接料2位）
    r_control(b"\x02", execute_time=execute_time)  # R轴停转
    temperature_control(b"\x02", execute_time=execute_time)  # 停火
...
def wash_control(execute_time: float = 0):
    ...
    temperature_control(b"\x01", 2000, execute_time=execute_time + 5)  # 加热200℃
    # 120秒后
    temperature_control(b"\x01", 2000, execute_time=execute_time + 125)  # 停止加热
    r_control(b"\x02", execute_time=execute_time + 125)  # R轴停转
    y_control(b"\x01", 7, execute_time=execute_time + 125)  # Y轴定位7号位（倒水位）
    # 10秒后
    y_control(b"\x01", 1, execute_time=execute_time + 135)  # Y轴定位1号位（接料1位）
